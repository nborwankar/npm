...
function registryInstall (pkglist, cb) {
  ...
  ;(function F (pkg) {
    ...
    if (tag) {
      ...
      var tagVersion = data["dist-tags"][tag]
      data = data.versions[tagVersion]
      if (!seen[name+"@"+data.version]) {
        // if that version doesn't work on this version of node, then complain,
        // but allow it anyway.  dependencies will work with a range, so that
        // won't be a problem.
        if (!data._nodeSupported) {
          log("", "WARNING !")
          log( data.name+"@"+tag
             + " not supported by node@"
             + process.version, "WARNING !")
          log("Installing it anyway, since you asked so nicely.", "WARNING !")
          log("", "WARNING !")
          unsupportedButAllowedAnyway[ data._id ] = true
        }
        pkglist.push(name+"@"+data.version)
      }
      return F(pkglist.shift())
    }
    
    // TODO: Make sure that the stuff getting added is _nodeSupported,
    // unless it was specifically requested in the original args, and then
    // warn about it.
